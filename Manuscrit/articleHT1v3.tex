...
%Indeed, we focus on a \YD{situation: encore de la traduction littérale!!!} representing South-Cameroon.
%\YD{A introduire plus tôt; cf ma remarque plus haut...}
%In this tropical region, some of the populations (as the Baka Pygmies) still rely on hunting and agriculture for their livelihood, see \cite{avila_martin_food_2024}. These populations do not threaten the vegetation (consummation of plant for food or medical uses are low compared to forest regeneration, see \cite{koppert_consommation_1996}). On the other hand, the forest is impacted by industrial development: a deep sea harbor was built near the city of Kribi and industrial palm groves are expanded. Moreover, these new constructions are supported by the development of roads, which fragmented the environment. See for example \cite{foonde_change_2018, romain_deforestation_2017} about landscape and biodiversity changes around Kribi.

%\YD{La aussi mal placé ce paragraphe: il doit arriver plus tôt...}
%\vdeux{
%This work aims to investigate the impact of overhunting and the anthropisation of habitat on the possibility of coexistence between a forest based human population and wildlife. It also highlights how the presence of a constant human immigration affects this possibility.
%}
...
%
%\begin{proof}
%We first prove that $\Omega_{eq, 2}$ is an invariant set. Since we already prove proposition \ref{prop:invariantRegion} about the invariance of $\Omega_{eq}$, it is sufficient to show that $(\nabla G_1 \cdot g)|_{h_D^*} \geq 0$, $(\nabla G_2 \cdot g)|_{f_W^{\max}} \geq 0$ and $(\nabla G_3 \cdot g)|_{h_W^*} \leq 0$ in $\Omega_{eq, 2}$, where
%
%$$
%G_1(z) = h_D - h_D^*,
%\quad
...
%Therefore, since $\N_{\cI > 0} = r_F(1-\alpha)\dfrac{1 + \beta m\dfrac{\cI}{\mu_D - f_D}}{\lfw m\dfrac{\cI}{\mu_D - f_D}} \leq 1$, it exists $T>0$ such that for all $t\geq T$, $r_F(1-\alpha) \dfrac{1-\beta h_W^g}{-h_W^g} \leq 1$. Now, using $-h_W^g \leq -h_W^{f_{eq}}$, we obtain that for all $t\geq T$, $r_F(1-\alpha) \dfrac{1-\beta h_W^{f_{eq}}}{-h_W^{f_{eq}}} \leq 1$. This gives that for all $t\geq T$, $\dfrac{df_W^{f_{eq}}}{dt} \leq 0$, and $f_W^{f_{eq}}$ converges towards 0. 
