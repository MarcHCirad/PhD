...
%
%\begin{remark}
%In the case where $\cI = 0$, the equilibrium of coexistence $EE^{HF_W}_{\cI=0}$ exists if $\lambda_{F, \cI=0}^{Min} < \lfw$, see remark \marc{ref}. When $\cI > 0$, the equilibrium of coexistence exists, according to previous proposition, only if $\lfw < \dfrac{r_F(\mu_D - f_D)}{m \cI} := \lambda_{F, \cI>0}^{Max}$. This will be interpret in the section \marc{ref}.
%\end{remark}
