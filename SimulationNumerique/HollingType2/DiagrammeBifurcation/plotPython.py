...
myFig, myAx = plotDiscreteBifurcationFile(
                    ...
                    , tickText=["$EE^{H}$ GAS", "$EE^{HF_W}$ GAS", "Limit Cycle ard \n $EE^{HF_W}$"]#, "H HFW", "H??"]
                    , dicEqColor={0 : "#ea801c", 1 : "#298c8c", 2 : "#C9FFFF"}#, 3:"green", 4 :"red"}
                    , transformData=eqNamestoNbr
                    )

# inputLambdaMin = pathWD + "/Diagram1D" + I + "Min.csv"
...
inputLambdaMax = pathWD + "/Diagram1D" + I + "Max.csv"
...
myAx.legend(fontsize=fontsize, loc="upper right", title="$\mathcal{I} = 0.1$", title_fontsize = fontsize)
myAx.set_xlim((0, 0.5))
