...
struct modelAlleeEffectPref <: mathematicalModel
    rF::Float64
    KF::Float64
    omega::Float64
    f::Float64
    ...
    function modelAlleeEffectPref(modelParam::Dict{String, Float64})
        ...
        new(rF, KF, omega, f, muF, lambdaFH, rV, KV, LV, alpha, muV, 
                    lambdaVH, rH, a, b, c, beta, tau, thresholdsName)
    end
end

function equationModel(model::modelAlleeEffectPref, variables::Vector{Float64})
    F, V, H = variables[1], variables[2], variables[3]
    dF = (model.rF * (1 - F / model.KF) * F - (1 - model.tau) * model.omega * model.f * H * F 
                - model.muF * F - model.tau * model.lambdaFH * F * H)

    dV = (model.rV * (1 - model.tau) * H / ((1 - model.tau) * H + model.LV) * 
            (1 - V / model.KV) * V - model.alpha * V * F - model.muV * V -
            (1 - model.tau) * model.lambdaVH * V * H)
...
function thresholdH(model::modelAlleeEffectPref ; F = 0, V =0)
    return (model.a * F + model.b * V + model.c) / model.beta
end

function computeHmin(model::modelAlleeEffectPref)
...
function computeBetaMax(model::modelAlleeEffectPref)
...
function thresholdF(model::modelAlleeEffectPref ; H = 0)
    return model.rF / (model.muF + H * (model.omega * model.f + model.lambdaFH))
end

function thresholdV(model::modelAlleeEffectPref, H::Float64 ; F = 0)
    fact1 = H / (model.LV + H)
    fact2 = model.rV / (model.lambdaVH * H + model.alpha * F + model.muV)
    return fact1 * fact2
end

function thresholdVi(model::modelAlleeEffectPref)
...
function thresholdDeltaVH(model::modelAlleeEffectPref)
...
function computeThresholds(model::modelAlleeEffectPref)
...
function interpretThresholds(model::modelAlleeEffectPref, thresholdsValues)
    listEq = ""
...
function eqTrivial(model::modelAlleeEffectPref)
    return (0, 0, 0)
end

function eqHBeta(model::modelAlleeEffectPref)
    return (0, 0, model.beta)
end

function eqH(model::modelAlleeEffectPref)
    return (0, 0, model.c)
end

function eqF(model::modelAlleeEffectPref)
    Feq = model.KF * ( 1 - model.muF / model.rF)
    return (Feq, 0, 0)
end

function eqFHBeta(model::modelAlleeEffectPref)
    Feq = model.KF * (1 - 
        (model.muF + model.beta*(model.omega*model.f + model.lambdaFH) / model.rF))
    return (Feq, 0, model.beta)
end

function eqFH(model::modelAlleeEffectPref)
...
function eqVH2(model::modelAlleeEffectPref)
    A = 1 + model.b * model.KV * model.lambdaVH / model.rV
    B = model.KV * (model.lambdaVH * (2 * model.c + model.LV) + model.muV)/model.rV - model.KV + 
            model.c / model.b
    C = model.KV * model.c / model.b *
...
function eqFVHBeta(model::modelAlleeEffectPref)
    Feq = model.KF * (1 - 
        (model.muF + model.beta * (model.omega * model.f + model.lambdaFH) / model.rF))
...
function eqFVH1(model::modelAlleeEffectPref)
...
function eqFVH2(model::modelAlleeEffectPref)
    FFH, _, HFH = eqFH(model) 
...
function modelTable(model::modelAlleeEffectPref; biologicalModel = false)
...
function computeBifurcationDiagram(model::modelAlleeEffectPref, 
    ...
    for ind_col in 2:length(listParam2)+1
        ...
        for ind_row in 2:length(listParam1)+1
            ...
            localModel = modelAlleeEffectPref(localParam)
